...
export class SimStatusWidget extends ReactWidget {
  ...
  async updateStatus( status : MosaikDockerSim.SimStatus ) : Promise<void> {
    ...
    if ( 1 != elements.length ) {
      console.error(
        `[mosaik-docker-jl] SimStatusWidget: wrong number of elements with class name 'jp-Widget' (${ elements.length } instead of 1 )`
      );
    }
    let contentDiv = elements[0];
...
export interface SimStatusComponentProps {
  model: IMosaikExtension;
}


export class SimStatusComponent extends React.Component<
  SimStatusComponentProps
> {

  /// Constructor.
  constructor( props: SimStatusComponentProps ) {
    super( props );
  }
    ...
    return (
      <div>
        <div className="jp-Widget">
          // This will be updated by calling SimStatusWidget's updateStatus method.
        </div>
        <div className="jp-Widget-button-span">
          <button
            className="jp-Dialog-button jp-mod-reject jp-mod-styled"
            onClick={ () =>
              this.props.model.displaySimStatus()
            }
          >
            Refresh
          </button>
